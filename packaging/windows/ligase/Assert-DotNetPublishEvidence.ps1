...
function Test-IdentitySnapshotDetail($Value, [string]$Stage, [string]$Reason) {
  if ($Stage -cne 'identitySnapshot') { return ($null -eq $Value) }
  Assert-ExactProperties $Value @('innerStage','reasonCode','attemptCount','firstListCount','secondListCount','remainingDeadlineTicks')
  $map = @{jobListFirst=@('jobUnavailable','listBounds','deadlineExceeded');openProcess=@('processDisappeared','accessDenied','identityReadFailed','deadlineExceeded');identityRead=@('processDisappeared','identityReadFailed','deadlineExceeded');memoryRead=@('processDisappeared','memoryReadFailed','deadlineExceeded');jobListSecond=@('jobUnavailable','listBounds','listDrift','deadlineExceeded');identityRecheck=@('processDisappeared','identityReadFailed','identityDrift','deadlineExceeded');aggregate=@('aggregateOverflow','deadlineExceeded');caller=@('deadlineExceeded','unexpectedCallerFailure')}
  if ($Value.innerStage -isnot [string] -or $Value.reasonCode -isnot [string] -or -not $map.ContainsKey($Value.innerStage) -or $Value.reasonCode -notin $map[$Value.innerStage] -or $Value.reasonCode -cne $Reason) { return $false }
  foreach ($name in @('attemptCount','firstListCount','secondListCount','remainingDeadlineTicks')) { if (-not (Test-JsonInteger $Value.$name) -or [long]$Value.$name -lt 0) { return $false } }
  return ([long]$Value.attemptCount -ge 1 -and [long]$Value.firstListCount -le 4096 -and [long]$Value.secondListCount -le 4096)
}
function Read-Terminal([string]$Path, [string]$ExpectedRunId) {
  ...
  Assert-ExactProperties $terminal @('schemaVersion','resultCode','state','primary','identitySnapshotDetail','execution','cleanup','streams','correlation','writtenUtc')
  ...
  $reasonPairs = @{none=@('none');processStart=@('processStartFailed');processExit=@('nonzeroExit');execution=@('runnerException');deadline=@('timeout');resourceLimit=@('privateBytesLimitExceeded');identitySnapshot=@('jobUnavailable','processDisappeared','accessDenied','identityReadFailed','memoryReadFailed','identityDrift','listBounds','listDrift','aggregateOverflow','deadlineExceeded','unexpectedCallerFailure');pipeDrain=@('stdoutReadFailed','stderrReadFailed','pipePending','outputOverflow');cleanup=@('rootStopFailed','descendantsRemaining','ownedPrivateBytesRemaining','cleanupDeadlineExceeded');ownership=@('ownershipHandleLost')}
...
if ($versionTerminal.state -cne 'passed' -or $versionTerminal.primary.stage -cne 'none' -or $versionTerminal.primary.reasonCode -cne 'none' -or [long]$versionTerminal.execution.exitCode -ne 0) { throw 'dotnetPublishEvidenceInvalid' }
if ($publishTerminal.primary.stage -ceq 'identitySnapshot') {
  if ($null -ne $Evidence.exitCode -or $null -ne $publishTerminal.execution.exitCode) { throw 'dotnetPublishEvidenceInvalid' }
} elseif (-not (Test-JsonInteger $Evidence.exitCode) -or [long]$publishTerminal.execution.exitCode -ne [long]$Evidence.exitCode) { throw 'dotnetPublishEvidenceInvalid' }
if ($publishTerminal.primary.stage -ceq 'identitySnapshot') { throw "dotnetPublishEvidenceUnavailable:identitySnapshot:$($publishTerminal.primary.reasonCode)" }
